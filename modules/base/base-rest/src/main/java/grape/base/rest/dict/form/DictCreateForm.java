...
public class DictCreateForm extends BaseForm {
    ...
    @ApiModelProperty(value = "是否为系统字典",notes = "一般系统字典代码中会做判断，不能修改或删除",required = true)
    private Boolean isSystem;

    @NotNull(message = "是否为公共字典不能为空")
    @ApiModelProperty(value = "是否为公共字典",notes = "如果为公共字典不限制使用，否则按相应数据权限查询",required = true)
    private Boolean isPublic;

    @NotNull(message = "是否为字典组不能为空")
    @ApiModelProperty(value = "是否为字典组",notes = "不是字典组就是字典项目，没有其它的",required = true)
